...
├── scripts/           # figure generators, table export, Paderborn download
...
- Windows / Linux / macOS. Run the commands below from the repository root `code01/`.
...
python main.py all          # all figures + tables (skips only Table VII, which is manuscript text)
```
...
Figure scripts also refresh `output/figures/`. Table export rebuilds `output/tables/` from experiment directories first (skip with `--no-refresh`).
...
| Tables S1–S5      | Degradation matrices, Cells O / A / B / C / D                      | five-cell `degradation_table_*.csv` + per-seed finals (Δ vs clean is recomputed) |
...
These are **new** runs. They must not overwrite `output/experiments/CellD_Paderborn_ATSF`, `nppad_atsf_full`, or the other Cell\* folders. Smoke every command on synthetic data first. Pin `torch==2.13.0` (same CUDA wheel as section 1). Stop and report versions if a rerun of an old seed drifts by more than **1 pp**.
...
```bash
# smoke (must pass before real data)
...
python -m atsf_dcg.run_experiments --cell replication --dataset nppad --arch tsnet \
  --synthetic --smoke --configs full "w/o_spectral" "w/o_gating" full_r2_gumbel
...
Then merge old (42–51) and new (52–71) seeds without touching the original folders:

```bash
python scripts/reanalyze_n30.py --smoke    # uses existing n=10; writes output/analysis_n30/
python scripts/reanalyze_n30.py            # same, plus n=30 dirs when they exist
```

`output/analysis_n30/` holds `tableII_n30.csv` (ablations), `tableIII_n30.csv` (remedies), `tableIV_n30.csv` (mechanism indicators), `tableVI_cellO_n30.csv`, `tableVI_cellA_n30.csv`, Holm/TOST family CSV, `d1_d2_recalibration.csv` (balanced 50-run subset **and** all available runs), and `tier_changes.md`.
...
├── figures/        # generator original filenames
