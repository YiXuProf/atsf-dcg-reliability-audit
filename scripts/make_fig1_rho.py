...
Usage (from the repository root)::
    python scripts/make_fig1_rho.py
    python scripts/make_fig1_rho.py --results-dir output/experiments/nppad_atsf_full \
        --out output/figures/fusion/fig1_rho_trajectories.png

Reads:
    <results-dir>/rho_curve.csv    (config, seed, epoch, rho)  -- written by
                                   run_experiments.py (training or --merge-only)
Writes:
    fig1_rho_trajectories.png (300 dpi) and fig1_rho_trajectories.svg (vector)
    (generator stem fig1_*; paper copy is fig02_* via main.py)

Also prints the per-config summary stats quoted in the paper text/caption
(median rho, % of epochs with rho>1.5, first-5 / last-5 epoch means) so the
numbers in Section 3.3.1 / Section 4.2 / Fig. 2 can be checked against the
plotted data.
"""
...
def main() -> None:
    ...
    ap.add_argument("--out",
                    default=str(FIGURES_ROOT / "fusion" / "fig1_rho_trajectories.png"))
    args = ap.parse_args()
