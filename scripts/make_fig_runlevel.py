"""Run-level panels for manuscript Figs. 3–7 (generator stems fig2–fig6).

Regenerates every figure from an ``atsf_dcg.eval_dump`` artefact directory
produced under the run-level leakage-free protocol::

    python scripts/make_fig_runlevel.py \
        --dump-dir output/intermediate/eval_dump/full_seed42 \
        --out-dir output/figures/fusion

Every figure is written as PNG (300 dpi) + SVG.  Key quantitative stats
(per-class alpha band, gate saturation rates, raw-vs-feature silhouette,
confusion-matrix accuracy) are printed to stdout for the paper text.
When the dump was made with ``--synthetic`` the raw test windows are
rebuilt from ``make_synthetic`` with the parameters stored in meta.json
(``--data-root`` is then not needed).
"""
...
def fig1_alpha_histogram(alpha: np.ndarray, out_dir: Path) -> None:
    ...
    _save(fig, out_dir, "fig2_fusion_weight_dist")
...
def fig2_perclass_alpha(alpha: np.ndarray, y_true: np.ndarray,
    ...
    ax.set_ylim(0, 1)
    _save(fig, out_dir, "fig3_fusion_weight_by_class")
...
def fig3_gate_heatmaps(gates: np.ndarray, preds: pd.DataFrame,
    ...
    _save(fig, out_dir, "fig4_gate_heatmap")
...
def fig5_confusion_matrix(preds: pd.DataFrame, class_names: list[str],
    ...
    _save(fig, out_dir, "fig6_confusion_matrix")
